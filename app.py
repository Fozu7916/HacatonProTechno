...
if role in ["СММ", "Руководитель", "Администратор"]:
    if role == "Администратор":
        with st.sidebar.expander("🔑 VK пользовательский токен (для постинга с файлами)", expanded=False):
            current_vk_user_token = get_setting("vk_user_token", "") or ""
            vk_user_token_in = st.text_input(
                "VK_USER_TOKEN (пользователь-админ)",
                value=current_vk_user_token,
                type="password",
                help="Нужен для публикации и загрузки вложений в VK. Токен сообщества не подходит.",
            )
            if st.button("Сохранить VK_USER_TOKEN", key="save_vk_user_token_btn"):
        ...
        with st.sidebar.expander("➕ Добавить Telegram-канал"):
            new_tg_channel_id = st.text_input("ID/username канала", key="new_tg_channel_id", placeholder="@my_channel или -100...")
            new_tg_channel_name = st.text_input("Название канала", key="new_tg_channel_name", placeholder="Название")
            new_tg_bot_token = st.text_input("Bot token", key="new_tg_bot_token", type="password")
            new_tg_tags = st.text_input("Теги канала (через запятую)", key="new_tg_tags", placeholder="школьные, студенческие")
            if st.button("Сохранить Telegram-канал", key="save_tg_channel_btn"):
                try:
                    cid = (new_tg_channel_id or "").strip()
                    cname = (new_tg_channel_name or cid or "Telegram канал").strip()
                    upsert_telegram_channel(cid, cname, (new_tg_bot_token or "").strip(), tags=(new_tg_tags or "").strip(), is_active=True)
                    st.sidebar.success(f"Telegram-канал {cname} сохранен")
                    st.rerun()
                except Exception as e:
                    st.sidebar.error(f"Ошибка сохранения Telegram-канала: {e}")
    ...
    available_tg_channels = get_telegram_channels(active_only=True)
    if available_tg_channels:
        all_tg_tags = sorted({tag for ch in available_tg_channels for tag in parse_group_tags(ch.get("tags"))})
        selected_tg_tags = st.sidebar.multiselect(
            "Теги Telegram-каналов",
            options=all_tg_tags,
            default=[],
            help="Можно выбрать несколько тегов",
        )
        filtered_tg_channels = available_tg_channels
        if selected_tg_tags:
            selected_tg_tag_set = set(selected_tg_tags)
            filtered_tg_channels = [
                ch for ch in available_tg_channels
                if selected_tg_tag_set.intersection(set(parse_group_tags(ch.get("tags"))))
            ]
        tg_options = [str(ch["id"]) for ch in filtered_tg_channels]
        tg_name_map = {str(ch["id"]): ch.get("name") or str(ch["id"]) for ch in available_tg_channels}
        tg_tags_map = {str(ch["id"]): parse_group_tags(ch.get("tags")) for ch in available_tg_channels}
        selected_telegram_ids = st.sidebar.multiselect(
            "Telegram-каналы",
            options=tg_options,
            default=tg_options,
            format_func=lambda cid: (
                f"{tg_name_map.get(cid, cid)} ({cid})"
                + (f" [{', '.join(tg_tags_map.get(cid, []))}]" if tg_tags_map.get(cid) else "")
            ),
            help="Выберите Telegram-каналы для публикации",
        )
...
if role in ["Руководитель", "Наблюдатель", "Администратор"]:
    ...
    if st.sidebar.button("🔄 Обновить данные из VK и Telegram"):
        with st.sidebar.status("Парсинг VK и Telegram..."):
            from parser.vk_parser import parse_all_posts
            from parser.telegram_parser import parse_telegram_updates
            from database.db import upsert_post
            data = parse_all_posts(n=200, group_ids=selected_group_ids or None)
            for item in data:
                upsert_post(item["post"])
            tg_saved = parse_telegram_updates(channel_ids=selected_telegram_ids or None, limit=300)
        st.sidebar.success(f"Обновлено! VK: {len(data)} постов, Telegram: {tg_saved} записей")
        st.rerun()
    ...
    if 'active_report' in st.session_state and st.session_state['active_report']:
        ...
        with st.expander("📋 Расширенный отчет для руководства", expanded=True):
            ...
            st.caption(
                f"Telegram за период: постов {report.get('TG постов', 0)}, "
                f"каналов {report.get('TG каналов', 0)}, "
                f"охват {report.get('TG охват (views)', 0)}"
            )
...
if role not in ["Руководитель", "Наблюдатель"]:
                ...
                if st.button("В очередь"):
                    if smm_text:
                        ...
                        add_to_queue(
                            ...
                            target_telegram_ids=",".join(selected_telegram_ids) if selected_telegram_ids else None,
                        )
    ...
    if role in ["СММ", "Руководитель", "Администратор"]:
        if st.button("🚀 Опубликовать следующий по приоритету"):
            ok = publish_next_post(
                target_groups=selected_group_ids,
                target_telegram_channels=selected_telegram_ids,
                allow_env_fallback=False,
            )
            if ok:
                st.success("Пост опубликован")
            else:
                st.error("Публикация не удалась. Проверьте права бота в Telegram-канале и корректность канала/token.")
            st.rerun()
...
with tab_a:
    ...
    st.subheader("Архив Telegram")
    conn = get_connection()
    try:
        tg_archive_df = pd.read_sql(
            """
            SELECT
                tp.published_at AS date,
                COALESCE(tc.name, tp.channel_id) AS `Канал`,
                LEFT(COALESCE(tp.text, 'Без текста'), 120) AS `Заголовок`,
                tp.text AS text,
                tp.attachment AS `Вложение`
            FROM telegram_posts tp
            LEFT JOIN telegram_channels tc ON tc.id = tp.channel_id
        ...
        tg_archive_df = pd.DataFrame(columns=["date", "Канал", "Заголовок", "text", "Вложение"])
    conn.close()
    st.dataframe(tg_archive_df, width='stretch')
